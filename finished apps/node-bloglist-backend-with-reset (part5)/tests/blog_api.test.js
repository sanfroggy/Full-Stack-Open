...
describe('when a delete request is made', () => {
    ...
    test('the first note created by a user is succesfully deleted', async () => {
        ...
        let response = await api.delete(`/api/blogs/${blogsAtStart[3].id}`)
            .set('Authorization', `bearer ${token}`)
            .expect(204)
        ...
        response = await api.delete(`/api/blogs/${blogsAtStart[3].id}`)
            .set('Authorization', `bearer ${token}`)
            .expect(500)
...
describe('when a post request is made to add a user', () => {
    ...
    test('user related blogs and blog related users are defined and ' +
