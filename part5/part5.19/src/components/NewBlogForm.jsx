...
const NewBlogForm = ({ createMethod }) => {
    ...
    return (
        <form onSubmit={addBlog}>
            ...
            <div>
                Title: <input id='titleInput' type='text' value={title}
                    onChange={event => setTitle(event.target.value)}
                    placeholder='Enter blog title here.'/>
            </div>
            <br/>
            <div>
                Author: <input id='authorInput' type="text" value={author}
                    onChange={event => setAuthor(event.target.value)} />
            </div>
            <br />
            <div>
                Url: <input id='urlInput' type='text' value={url}
                    onChange={event => setUrl(event.target.value)}
                    placeholder='Enter blog url here.'/>
            </div>
            <br />
            <div>
                <button id="newBlogBtn" type="submit">Create</button>
            </div>
        </form>
    )
}
