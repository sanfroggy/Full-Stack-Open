...
const SingleBlog = ({ loggedUser }) => {
    ...
    /*If a blog no longer exists, but is still set as the blog to
    be displayed, useNavigate is also used to take the user back to
    the "homepage". */
    if (!blog) {
        navigate('/')
    }

    /*If loggedUser and blog have proper defined values the details
    of the blog are returned, along with a like button and a remove button
    if the user is the same that added the blog. */
    if (loggedUser && blog) {
        return (
            <div>
