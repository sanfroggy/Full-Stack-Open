...
const mostLikes = (blogs) => {
    if (blogs.length > 0) {
        const authors = [...new Set(blogs.map((blog) => blog.author))]
        let likecount = Array(authors.length).fill(0)
        ...
        const mostlikes = likecount.indexOf(likecount.reduce((a, b) => Math.max(a, b), -Infinity))

        const mostLikedAuthor = {
            author: authors[mostlikes],
            likes: likecount[mostlikes],
        }
